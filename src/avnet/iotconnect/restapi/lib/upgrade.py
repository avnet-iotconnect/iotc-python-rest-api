...
def query(query_str: str = '[*]') -> list[Firmware]:
    response = request(apiurl.ep_firmware, '/Firmware/lookup')
...
def create(
    ...
    ret = response.data.get(dc=FirmwareCreateResult)
    return ret
...
def delete_match_guid(guid: str) -> None:
    ...
    response = request(apiurl.ep_firmware, f'/Firmware/{guid}', method=HTTPMethod.DELETE)
